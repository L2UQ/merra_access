...
from numpy import ndarray, ma, random
import pandas
import math
import glob2
...
def merra_subset_2d( srchdr, srchdt, lnsq, ltsq, varnm, tmidx = 0, mflstr = 'inst1_2d_asm'):
    ### Extract a MERRA 2D variable from spatial subset
    ### tmidx:      Optional time index for diurnal sampling, single time slice is default
    ...
    while ( (j < len(flst)) and ((f2d < 0) or (f3d < 0) ) ):
        if ( (mflstr in flst[j]) and (dtstr in flst[j]) ):
            ...
            f = Dataset(mer2d,'r')
            vararr = f.variables[varnm][tmidx,ltsq,lnsq]
            f.close()
        j = j + 1
...
def merra_conv_temp_prof( srchdr, srchdt, lnsq, ltsq, lvout, tmidx = 0, msgval=-9999.):
    ...
    while ( (j < len(flst)) and ((f2d < 0) or (f3d < 0) ) ):
        if ( ('inst1_2d_asm' in flst[j]) and (dtstr in flst[j]) ):
            ...
            mer2d = '%s/%s' % (srchdr,flst[j])
            f = Dataset(mer2d,'r')
            t2m = f.variables['T2M'][tmidx,ltsq,lnsq]
            psfc = f.variables['PS'][tmidx,ltsq,lnsq]
        ...
        if ( ('inst3_3d_asm' in flst[j]) and (dtstr in flst[j]) ):
            ...
            f = Dataset(mer3d,'r')
            tmpmer = f.variables['T'][tmidx,:,ltsq,lnsq]
...
def merra_conv_shum_prof( srchdr, srchdt, lnsq, ltsq, lvout, tmidx = 0, vrnm = 'QV', msgval=-9999.):
    ...
    while ( (j < len(flst)) and ((f2d < 0) or (f3d < 0) ) ):
        ...
        if ( ('inst3_3d_asm' in flst[j]) and (dtstr in flst[j]) ):
            ...
            mer3d = '%s/%s' % (srchdr,flst[j])
            f = Dataset(mer3d,'r')
            qvmer = f.variables[vrnm][tmidx,:,ltsq,lnsq]
            lvmer = f.variables['lev'][:]
            f.close()
        j = j + 1
...
def rh_from_qv_prof( qvprf, tprf, plvs, msgval=-9999.):
    ...
    for k in range(nlvout):
        ...
        tprftmp = ma.zeros((ny,nx),dtype=numpy.float32)
        tprftmp[:,:] = tprf[k,:,:]
        tprftmp = ma.masked_where(tprftmp <= 0, tprftmp)

        # Calculate sat vap pres in hPa, AMS Glossary
        #es = lcmsk * (6.112 * numpy.exp(17.67 * (tprftmp-273.15) / (tprftmp - 29.65)) )  + msgval * (1.0 - lcmsk)
        es = 6.112 * numpy.exp(17.67 * (tprftmp-273.15) / (tprftmp - 29.65))
        mmrs = lcmsk * (0.622 * es / plvs[k]) + msgval * (1.0 - lcmsk)
        rhout[k,:,:] = lcmsk * (qvprf[k,:,:] / mmrs) + msgval * (1.0 - lcmsk)

    return rhout

def merra_conv_heights( srchdr, srchdt, lnsq, ltsq, lvout, tprf, tmidx = 0, vrnm = 'H', sfcht = 'PHIS', msgval=-9999.):
    ...
    while ( (j < len(flst)) and ((f2d < 0) or (f3d < 0) ) ):
        ...
        if ( ('inst3_3d_asm' in flst[j]) and (dtstr in flst[j]) ):
            ...
            mer3d = '%s/%s' % (srchdr,flst[j])
            f = Dataset(mer3d,'r')
            htmer = f.variables[vrnm][tmidx,:,ltsq,lnsq]
            lvmer = f.variables['lev'][:]
            phisfc = f[sfcht][tmidx,ltsq,lnsq] / 9.8
            f.close()
        j = j + 1
...
def merra_conv_cfrac_prof( srchdr, srchdt, lnsq, ltsq, lvout, tmidx = 0, vrnm = 'CLOUD', msgval=-9999.):
    ...
    while ( (j < len(flst)) and ((f2d < 0) or (f3d < 0) ) ):
        ...
        if ( ('tavg3_3d_rad' in flst[j]) and (dtstr in flst[j]) ):
            ...
            mer3d = '%s/%s' % (srchdr,flst[j])
            f = Dataset(mer3d,'r')
            cfmer = f.variables[vrnm][tmidx,:,ltsq,lnsq]
            lvmer = f.variables['lev'][:]
            f.close()
        j = j + 1
...
def setup_airs_cloud(flnm, tms, lats, lons, tmunit = 'Seconds since 1993-01-01 00:00:00', airstq=False):
    ...
    # tmunit:  Text for time variable units
    # airstq:  Optionally include AIRS temperature, humidity profiles
    ...
    if airstq: 
        dimlvl = qout.createDimension('lev',nlvl)
        dimh2o = qout.createDimension('H2Olev',nh2o)
    ...
    varcqc2 = qout.createVariable('AIRS_TSfcStd_QC','i2',['time','lat','lon'], fill_value = -99)
    varcqc2.long_name = 'AIRS near-surface temperature quality flag'
    varcqc2.units = 'unitless'
    varcqc2.missing_value = -99

    # AIRS indexing
    vargrn = qout.createVariable('AIRS_granule','i2',['time','lat','lon'], fill_value = -99)
    vargrn.long_name = 'AIRS granule number'
    vargrn.units = 'unitless'
    vargrn.missing_value = -99

    varatrk = qout.createVariable('AIRS_atrack','i2',['time','lat','lon'], fill_value = -99)
    varatrk.long_name = 'AIRS along track index'
    varatrk.units = 'unitless'
    varatrk.missing_value = -99

    varxtrk = qout.createVariable('AIRS_xtrack','i2',['time','lat','lon'], fill_value = -99)
    varxtrk.long_name = 'AIRS cross track index'
    varxtrk.units = 'unitless'
    varxtrk.missing_value = -99

    if airstq:
        vartair = qout.createVariable('AIRS_TAir','f4',['time','lev','lat','lon'], fill_value = -9999)
        vartair.long_name = 'AIRS air temperature'
        vartair.units = 'K'
        vartair.missing_value = -9999

        varrh = qout.createVariable('AIRS_RH','f4',['time','H2Olev','lat','lon'], fill_value = -9999)
        varrh.long_name = 'AIRS relative humidity'
        varrh.units = 'unitless'
        varrh.missing_value = -9999

        vartsfc = qout.createVariable('AIRS_TSfc','f4',['time','lat','lon'], fill_value = -9999)
        vartsfc.long_name = 'AIRS surface skin temperature'
        vartsfc.units = 'K'
        vartsfc.missing_value = -9999
...
def airs_cfrac_match_merra(flnm, tmidx, tmday, lats, lons,  msgvl = -9999, \
                           l2srch = '/archive/AIRSOps/airs/gdaac/v7', airstq = False):
    # Set up matched AIRS/MERRA cloud file
    # flnm:    Name of output file
    ...
    # lons:    Longitude variable array
    # msgvl:   Missing value
    # l2srch:  Local search path for AIRS products
    # airstq:  Optionally include AIRS temperature, humidity profiles; will require airs2sup directory

    # Search AIRS Level 2
    nlvl = 101
    nh2o = 15
    airsdr = '%s/%04d/%02d/%02d/airs2ret' % (l2srch,tmday.year,tmday.month,tmday.day)
    airssp = '%s/%04d/%02d/%02d/airs2sup' % (l2srch,tmday.year,tmday.month,tmday.day)
    ...
    tsfqcarr = numpy.zeros((nlat,nlon),dtype=numpy.int16) - 99
    ncldarr = numpy.zeros((nlat,nlon,9),dtype=numpy.int16) - 99
    airsgrn = numpy.zeros((nlat,nlon),dtype=numpy.int16) - 99
    airsatrk = numpy.zeros((nlat,nlon),dtype=numpy.int16) - 99
    airsxtrk = numpy.zeros((nlat,nlon),dtype=numpy.int16) - 99

    if airstq:
        tairarr = numpy.zeros((nlat,nlon,nlvl),dtype=numpy.float32) + msgvl
        tsfcarr = numpy.zeros((nlat,nlon),dtype=numpy.float32) + msgvl
        rharr = numpy.zeros((nlat,nlon,nh2o),dtype=numpy.float32) + msgvl
    ...
    if (len(asclst) > 0):
        ...
        for j in range(len(asclst)):
            ...
            tsfcl2 = ncl2.variables['TSurfStd'][:,:]
            rhl2 = ncl2.variables['RelHum'][:,:,:]
            ncldair = ncl2.variables['nCld'][:,:,:,:]
            ncl2.close()

            nairtrk = l2lat.shape[0]
            nairxtk = l2lat.shape[1]
            crgrn = int(asclst[j][16:19])
            ...
            supfl = ''
            if airstq:
                glbstr = '%s/%s*.hdf' % (airssp,asclst[j][0:23])
                print(glbstr)
                glblst = glob2.glob(glbstr)
                print(glblst)
                if len(glblst) > 0:
                    supfl = glblst[0]
            #l2lat = ncl2.variables['Latitude'][:,:]
            ...
            if (airstq and (supfl != '') and os.path.isfile(supfl)):
                ncsp = Dataset(supfl)
            for k in range(l2mrg.shape[0]):
                ...
                tsfqcarr[yidxout,xidxout] = tsfcqc[yidxl2,xidxl2].astype(numpy.int16)
                airsatrk[yidxout,xidxout] = yidxl2
                airsxtrk[yidxout,xidxout] = xidxl2
                airsgrn[yidxout,xidxout] = crgrn

                if airstq:
                    tsfcarr[yidxout,xidxout] = tsfcl2[yidxl2,xidxl2].astype(numpy.float32) 
                    rharr[yidxout,xidxout,:] = rhl2[yidxl2,xidxl2,:].astype(numpy.float32) 
                    if ( (supfl != '') and os.path.isfile(supfl)):
                        tairarr[yidxout,xidxout,1:101] = ncsp.variables['TAirSup'][yidxl2,xidxl2,:]
    ...
    varcfqct = qout.variables['AIRS_TSfcStd_QC']
    varcfqct[tmidx,:,:] = tsfqcarr[:,:]

    vargrn = qout.variables['AIRS_granule']
    vargrn[tmidx,:,:] = airsgrn

    varatrk = qout.variables['AIRS_atrack']
    varatrk[tmidx,:,:] = airsatrk

    varxtrk = qout.variables['AIRS_xtrack']
    varxtrk[tmidx,:,:] = airsxtrk

    if airstq:
        vartsfc = qout.variables['AIRS_TSfc']
        vartsfc[tmidx,:,:] = tsfcarr[:,:]

        vartair = qout.variables['AIRS_TAir']
        vartair[tmidx,:,:,:] = tairarr[:,:,:]

        varrh = qout.variables['AIRS_RH']
        varrh[tmidx,:,:,:] = rharr[:,:,:]
        ...
        for i in range(nlon):
            ...
            if (nslb[j,i] > 0):
                ...
                if (lvfr.shape[0] == lvsq.shape[0]):
                    ...
                    #print('Sfc Frz Level')
                elif (lvmer[lvfr[0]-1] > 0.01*psfc[j,i]):
                    frzlvl = psfc[j,i] * 0.01
                else:
                    if (ma.is_masked(tmpmer[lvfr[0]-1,j,i])):
                        frzlvl = psfc[j,i] * 0.01
                    else:
                ...
                if (qism[j,i] > 1):
                    ...
                    for sl1 in range(nice):
                        ...
                        for slw in range(lst1,lst2):
                            ...
                            if (ma.is_masked(qimer[slw,j,i]) and ma.is_masked(qimer[slw+1,j,i])):
                                iwp[sl1] = iwp[sl1] + (0.0 + 0.0) * dlprs / 9.8
                            elif ma.is_masked(qimer[slw,j,i]):
                                iwp[sl1] = iwp[sl1] + (0.0 + qimer[slw+1,j,i]) * dlprs / 9.8
                            else:
                ...
                if (qlsm[j,i] > 1):
                    ...
                    for sl1 in range(nlqd):
                        ...
                        for slw in range(lst1,lst2):
                            dlprs = (lvmer[slw] - lvmer[slw+1]) * 100.0
                            if (ma.is_masked(qlmer[slw,j,i]) and ma.is_masked(qlmer[slw+1,j,i])):
                                lwp[sl1] = lwp[sl1] + (0.0 + 0.0) * dlprs / 9.8
                            elif ma.is_masked(qlmer[slw,j,i]):
                                lwp[sl1] = lwp[sl1] + (0.0 + qlmer[slw+1,j,i]) * dlprs / 9.8
                            else:
                ...
                if ( (nice > 0) and (nlqd >  0)):
                    cldfrm = pandas.concat([icefrm,wtrfrm],ignore_index=True)
                elif (nice > 0):
                    cldfrm = icefrm
                elif (nlqd > 0):
                    cldfrm = wtrfrm
                        ...
                        if chg2:
                            ...
                            if lvrg.shape[0] == 0:
                                ...
                                lwwt = (numpy.log(ptp2) - numpy.log(lvmer[lst2+1])) / \
                                       (numpy.log(lvmer[lst2]) - numpy.log(lvmer[lst2+1]))
                                hiwt = 1.0 - lwwt
                                if ma.is_masked(tmpmer[lst2,j,i]):
                                    cldslc['CTTemp'].values[1] = tmpmer[lst2+1,j,i]
                                else:
                                ...
                                if (ma.is_masked(pth2)):
                                    cldslc['WtrPath'].values[1] = 0.0
                                else:
            ...
            if slbct >= 1:
                ...
                if ( (numpy.isnan(cngwt1[j,i])) or (cngwt1[j,i] < 0.0) ):
                    flgstr = 'NGWater undefined at lat %d, lon %d' % (j,i)
                    print(flgstr)
                    print(cldslc)
                if ( (numpy.isnan(cttp1[j,i])) or (cttp1[j,i] < 0.0) ):
                    flgstr = 'CloudTemp undefined at lat %d, lon %d' % (j,i)
                    print(flgstr)
                    print(cldslc)
            if slbct == 2:
                ...
                if ( (numpy.isnan(cngwt2[j,i])) or (cngwt2[j,i] < 0.0) ):
                    flgstr = 'NGWater undefined at lat %d, lon %d' % (j,i)
                    print(flgstr)
                    print(cldslc)
                if ( (numpy.isnan(cttp2[j,i])) or (cttp2[j,i] < 0.0) ):
                    flgstr = 'CloudTemp undefined at lat %d, lon %d' % (j,i)
                    print(flgstr)
                    print(cldslc)
...
def airs_granule_overlap(mtfl, yrchc, mnchc, dychc, grnchc, mskvr, mskvl, \
                           l2srch = '/archive/AIRSOps/airs/gdaac/v7'):
...
def airs_granule_overlap_df(mtfl, yrchc, mnchc, dychc, grnchc, mskvr, mskvl, \
                           l2srch = '/archive/AIRSOps/airs/gdaac/v7'):
    ...
    if l2fd >= 0:
        ...
        l2mrg = pandas.merge(l2frm,mersb,on=['GridLon','GridLat'])
        if l2mrg.shape[0] > 0:
            # Additional summary (by lat/lon) here?
            grprw = l2mrg.groupby('L2RowIdx')
            rwcts = grprw.apply(df_qsmry_mltvr,grpvr='L2RowIdx',measlst=['L2Lon','L2Lat'])
            rwcts.reset_index(drop=True,inplace=True)
...
def df_qsmry_mltvr(dtfrm,grpvr,measlst):
    # Return quantile summary for list of variables 
    # dtfrm:    Input data frame
    # grpvr:    Grouping variable
    # measlst:  List of variables to summarize

    fct = dtfrm.shape[0]
    xvl = dtfrm[grpvr].values[0]
    plvs = numpy.array([25.0,50.0,75.0])
    for j in range(len(measlst)):
        tmpdt = dtfrm[measlst[j]]
        dtvld = tmpdt[numpy.isfinite(tmpdt)]
        dtqs = numpy.nanpercentile(dtvld,q=plvs)
        strq1 = '%s_Q25' % (measlst[j])
        strq2 = '%s_Q50' % (measlst[j])
        strq3 = '%s_Q75' % (measlst[j])
        if j == 0:
            dfout = pandas.DataFrame({grpvr : xvl, 'Count' : fct, \
                                      strq1 : dtqs[0], strq2 : dtqs[1], strq3 : dtqs[2]}, \
                                      index=[0])
        else:
            dfout[strq1] = dtqs[0]
            dfout[strq2] = dtqs[1]
