# Process MERRA for CONUS templates
...
dst = datetime.date(2019,9,1)
dfn = datetime.date(2019,12,1)
ddf = dfn - dst
totdy = ddf.days
...
for t1 in range(0,totdy):
    ...
    tmparr = merra_support.merra_conv_temp_prof(dtdr, d2, lnsb, ltsb, airs_sarta_levs)
    tmpout[t1,:,:,:] = tmparr[:,:,:]

    qvarr = merra_support.merra_conv_shum_prof(dtdr, d2, lnsb, ltsb, airs_sarta_levs, vrnm='QV')
    qvout[t1,:,:,:] = qvarr[:,:,:]
    ...
    psfc = merra_support.merra_subset_2d(dtdr, d2, lnsb, ltsb, 'PS')
    psfout[t1,:,:] = 0.01 * psfc[:,:]
    t2m = merra_support.merra_subset_2d(dtdr, d2, lnsb, ltsb, 'T2M')
    t2mout[t1,:,:] = t2m[:,:]
    tsfc = merra_support.merra_subset_2d(dtdr, d2, lnsb, ltsb, 'TS')
    tsfcout[t1,:,:] = t2m[:,:]
    zsfc = merra_support.merra_subset_2d(dtdr, d2, lnsb, ltsb, 'PHIS', mflstr = 'inst3_3d_asm')
    zsfmout[t1,:,:] = zsfc[:,:] / 9.8
    cftot = merra_support.merra_subset_2d(dtdr, d2, lnsb, ltsb, 'CLDTOT', mflstr = 'tavg1_2d_rad')
    cftotout[t1,:,:] = cftot[:,:] 

    htarr = merra_support.merra_conv_heights(dtdr, d2, lnsb, ltsb, airs_sarta_levs, tmparr)
    htout[t1,:,:,:] = htarr[:,:,:]
...
outnm = '%s/interpolated_merra2_for_SARTA_two_slab_%04d_%s_CONUS_NCA_%02dUTC.nc' % (outdr,dst.year,sstr,hrs)
