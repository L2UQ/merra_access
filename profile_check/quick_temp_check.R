...
ttmp = trpf[,,95,2]
ptmp = psfc[,,31]
lnsb = lnidx[ (ttmp > 350) & !is.na(ttmp)]
ltsb = ltidx[ (ttmp > 350) & !is.na(ttmp)]
...
# Plot
srtfrm = data.frame(Temp=trpf[xidx,yidx,lvsqsrt,tidx],Level=lvsrt[lvsqsrt])
srtfrm$LogRev = 7 - log(srtfrm$Level)
merfrm = data.frame(Temp=tmpmer[xsb,ysb,lvsqmer],Level=lvmer[lvsqmer])
merfrm$LogRev = 7 - log(merfrm$Level)
t2frm = data.frame(Temp=t2mer[xsb,ysb],Level=psfmer[xsb,ysb]*0.01)
...
gttl = sprintf("Temp Profile %.2f, %.2f",lnsrt[xidx],ltsrt[yidx])
gtmp = ggplot(srtfrm,aes(x=Temp,y=LogRev)) + geom_point() + 
